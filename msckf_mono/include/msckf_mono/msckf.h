...
namespace msckf_mono {
    ...
    private:
        ...
        std::vector<Vector3 < _S>, Eigen::aligned_allocator<Vector3 < _S>>>
        map_;
    ...
    public:
        ...
        void propagate(imuReading <_S> &measurement_) {
            ...
            Phi_.template block<3, 3>(0, 0) = imu_state_prop.q_IG.toRotationMatrix() * R_kk_1.transpose();
            ...
            tmp = measurement_.dT * imu_state_.v_I_G_null + imu_state_.p_I_G_null - imu_state_prop.p_I_G;
            ...
            Matrix<_S, 15, 15> imu_covar_prop =
                    Phi_ * (imu_covar_ + G_ * noise_params_.Q_imu * G_.transpose() * measurement_.dT) * Phi_.transpose();
        ...
        void augmentState(const int &state_id, const _S &time) {
            ...
            cam_state.p_C_G = imu_state_.p_I_G + imu_state_.q_IG.inverse() * camera_.p_C_I;
            ...
            J.template block<3, 3>(3, 0) = vectorToSkewSymmetric(imu_state_.q_IG.inverse() * camera_.p_C_I);
            J.template block<3, 3>(3, 12) = Matrix3<_S>::Identity();

            // Camera<_S> State Jacobian
            // MatrixX<_S> J = calcJ(imu_state_, cam_states_);

            MatrixX<_S> tempMat = MatrixX<_S>::Identity(15 + 6 * cam_states_.size() + 6, 15 + 6 * cam_states_.size());
            tempMat.block(15 + 6 * cam_states_.size(), 0, 6, 15 + 6 * cam_states_.size()) = J;
        ...
        void update(
            ...
            for (auto feature_id : tracked_feature_ids_) {
                ...
                auto input_feature_ids_iter = find(feature_ids.begin(), feature_ids.end(), feature_id);
                ...
                if (is_valid) {
                    size_t feature_ids_dist = distance(feature_ids.begin(), input_feature_ids_iter);
                ...
                // residualized
                if (!is_valid || (track->observations.size() >= msckf_params_.max_track_length)) {
                    featureTrackToResidualize <_S> track_to_residualize;
                    removeTrackedFeature(feature_id, track_to_residualize.cam_states, track_to_residualize.cam_state_indices);

                    // If track is long enough, add to the residualized list
                    if (track_to_residualize.cam_states.size() >= msckf_params_.min_track_length) {
            ...
            for (auto feature_id : tracks_to_remove_) {
                ...
                auto corresponding_id = std::find(tracked_feature_ids_.begin(), tracked_feature_ids_.end(), feature_id);
        ...
        void addFeatures(
                const std::vector<Vector2 < _S>, Eigen::aligned_allocator<Vector2 < _S>>>& features,
                const std::vector<size_t> &feature_ids) {
        ...
        void marginalize() {
            if (!feature_tracks_to_residualize_.empty()) {
                ...
                for (auto track = feature_tracks_to_residualize_.begin();
                    ...
                    bool isvalid = initializePosition(track->cam_states, track->observations, p_f_G);
                    ...
                    Vector3<_S> p_f_C1 = (track->cam_states[0].q_CG.toRotationMatrix()) * (p_f_G - track->cam_states[0].p_C_G);
                ...
                MatrixX<_S> H_o = MatrixX<_S>::Zero(2 * total_nObs - 3 * num_passed, 15 + 6 * cam_states_.size());
                MatrixX<_S> R_o = MatrixX<_S>::Zero(2 * total_nObs - 3 * num_passed, 2 * total_nObs - 3 * num_passed);
                ...
                for (int iter = 0; iter < feature_tracks_to_residualize_.size(); iter++) {
                    ...
                    if (gatingTest(H_o_j, r_o_j, track.cam_states.size() - 1)) {
                        ...
                        R_o.block(stack_counter, stack_counter, R_o_j.rows(), R_o_j.cols()) = R_o_j;
        ...
        void pruneRedundantStates() {
            ...
            for (auto &feature : feature_tracks_) {
                ...
                for (const auto &cam_id : rm_cam_state_ids) {
                    auto obs_it = find(feature.cam_state_indices.begin(), feature.cam_state_indices.end(), cam_id);
                ...
                if (involved_cam_state_ids.size() == 1) {
                    ...
                    feature.cam_state_indices.erase(feature.cam_state_indices.begin() + obs_id);
                    continue;
                }
                ...
                if (!feature.initialized) {
                    ...
                    if (!checkMotion(feature.observations.front(), feature_associated_cam_states)) {
                        for (const auto &cam_id : involved_cam_state_ids) {
                            auto cam_it = find(feature.cam_state_indices.begin(), feature.cam_state_indices.end(), cam_id);
                            if (cam_it != feature.cam_state_indices.end()) {
                                size_t obs_idx = distance(feature.cam_state_indices.begin(), cam_it);
                        ...
                        Vector3<_S> p_f_G;
                        if (!initializePosition(feature_associated_cam_states, feature.observations, p_f_G)) {
                            for (const auto &cam_id : involved_cam_state_ids) {
                                auto cam_it = find(feature.cam_state_indices.begin(), feature.cam_state_indices.end(), cam_id);
                                if (cam_it != feature.cam_state_indices.end()) {
                                    size_t obs_idx = distance(feature.cam_state_indices.begin(), cam_it);
            ...
            for (auto &feature : feature_tracks_) {
                ...
                for (const auto &cam_id : rm_cam_state_ids) {
                    ...
                    if (cam_it != feature.cam_state_indices.end()) {
                        involved_cam_state_ids.push_back(cam_id);
                        involved_observations.push_back(feature.observations[distance(feature.cam_state_indices.begin(), cam_it)]);
                    }
                }
                ...
                for (const auto &cam_state : cam_states_) {
                    if (find(involved_cam_state_ids.begin(), involved_cam_state_ids.end(), cam_state.state_id)
                          != involved_cam_state_ids.end()) {
                ...
                VectorX<_S> r_j = calcResidual(feature.p_f_G, involved_cam_states, involved_observations);
                ...
                if (gatingTest(H_x_j, r_x_j, nObs - 1)) {
                    ...
                    R_x.block(stack_counter, stack_counter, R_x_j.rows(), R_x_j.cols()) = R_x_j;
                ...
                for (const auto &cam_id : involved_cam_state_ids) {
                    ...
                    if (cam_it != feature.cam_state_indices.end()) {
                        ...
                        feature.observations.erase(
                                feature.observations.begin() + distance(feature.cam_state_indices.begin(), cam_it));
                    }
                }
            }
        ...
        void finish() {
            for (size_t i = 0; i < tracked_feature_ids_.size(); i++) {
                ...
                removeTrackedFeature(tracked_feature_ids_[i], camStatesTemp, camStateIndices);
                ...
                if (camStatesTemp.size() >= msckf_params_.min_track_length) {
                    ...
                    if (feature_tracks_[i].feature_id != tracked_feature_ids_[i]) {
                        for (typename std::vector<featureTrack<_S>>::iterator feat_track = feature_tracks_.begin();
        ...
        inline std::vector<Vector3 < _S>, Eigen::aligned_allocator<Vector3 < _S>>>
        ...
        inline std::vector<camState < _S>> getPrunedStates()
        {
            std::sort(pruned_states_.begin(), pruned_states_.end(),
                      [](camState <_S> a, camState <_S> b) {return a.state_id < b.state_id;});
    ...
    private:
        ...
        inline void calcF(const imuState <_S> &imu_state_k, const imuReading <_S> &measurement_k) {
        ...
        void calcMeasJacobian(const Vector3 <_S> &p_f_G,
            ...
            MatrixX<_S> H_x_j = MatrixX<_S>::Zero(2 * camStateIndices.size(), 15 + 6 * cam_states_.size());

            for (int c_i = 0; c_i < camStateIndices.size(); c_i++) {
                size_t index = camStateIndices[c_i];
                Vector3<_S> p_f_C = cam_states_[index].q_CG.toRotationMatrix() * (p_f_G - cam_states_[index].p_C_G);
                ...
                Matrix<_S, 2, 6> A;
                A << J_i * vectorToSkewSymmetric(p_f_C), -J_i * cam_states_[index].q_CG.toRotationMatrix();
                ...
                Matrix<_S, 2, 6> H_x = A - A * u * (u.transpose() * u).inverse() * u.transpose();
        ...
        VectorX <_S> calcResidual(
            ...
            VectorX <_S> r_j = VectorX<_S>::Constant(2 * camStates.size(), std::numeric_limits<_S>::quiet_NaN());
        ...
        bool checkMotion(
                const Vector2 <_S> first_observation,
            ...
            for (auto second_cam_iter = cam_states.begin() + 1; second_cam_iter != cam_states.end(); second_cam_iter++) {
                Isometry3 <_S> second_cam_pose;
                second_cam_pose.linear() = second_cam_iter->q_CG.toRotationMatrix().transpose();
                ...
                Vector3 <_S> translation = second_cam_pose.translation() - first_cam_pose.translation();
                // translation = translation / translation.norm();
                _S parallel_translation = translation.transpose() * feature_direction;
                Vector3 <_S> orthogonal_translation = translation - parallel_translation * feature_direction;
        ...
        void cost(const Isometry3 <_S> &T_c0_ci, const Vector3 <_S> &x, const Vector2 <_S> &z, _S &e) const {
            // Compute hi1, hi2, and hi3 as Equation (37).
            const _S &alpha = x(0);
            const _S &beta = x(1);
            const _S &rho = x(2);

            Vector3<_S> h = T_c0_ci.linear() * Vector3<_S>(alpha, beta, 1.0) + rho * T_c0_ci.translation();
            _S &h1 = h(0);
            _S &h2 = h(1);
            _S &h3 = h(2);
        ...
        void findRedundantCamStates(std::vector<size_t> &rm_cam_state_ids) {
            ...
            for (int i = 0; i < num_over_max; i++) {
                if (rm_cam_state_ids.end() ==
                    std::find(rm_cam_state_ids.begin(), rm_cam_state_ids.end(), cam_states_[i].state_id)) {
        ...
        bool gatingTest(const MatrixX <_S> &H, const VectorX <_S> &r, const int &dof) {
            ...
            MatrixX<_S> P2 = noise_params_.u_var_prime * MatrixX<_S>::Identity(H.rows(), H.rows());
        ...
        bool initializePosition(
            ...
            Vector3 <_S> final_position(solution(0) / solution(2), solution(1) / solution(2), 1.0 / solution(2));

            // Check if the solution is valid. Make sure the feature
            // is in front of every camera frame observing it.
            bool is_valid_solution = true;
            for (const auto &pose : cam_poses) {
                Vector3 <_S> position = pose.linear() * final_position + pose.translation();
                if (position(2) <= 0) {
            ...
            _S normalized_cost = total_cost / (2 * cam_poses.size() * cam_poses.size());
        ...
        void jacobian(const Isometry3 <_S> &T_c0_ci,
                      ...
                      Matrix<_S, 2, 3> &J, Vector2 <_S> &r, _S &w) const {
            // Compute hi1, hi2, and hi3 as Equation (37).
            const _S &alpha = x(0);
            const _S &beta = x(1);
            const _S &rho = x(2);
        ...
        void measurementUpdate(const MatrixX <_S> &H_o, const VectorX <_S> &r_o, const MatrixX <_S> &R_o) {
            if (r_o.size() != 0) {
                // Build MSCKF covariance matrix
                MatrixX<_S> P = MatrixX<_S>::Zero(15 + cam_covar_.rows(), 15 + cam_covar_.cols());
                P.template block<15, 15>(0, 0) = imu_covar_;
                ...
                for (size_t c_i = 0; c_i < cam_states_.size(); c_i++) {
                    Quaternion<_S> q_CG_up = buildUpdateQuat(deltaX.template segment<3>(15 + 6 * c_i)) * cam_states_[c_i].q_CG;
                ...
                // TODO: Check here
                cam_covar_ = P_corrected.block(15, 15, P_corrected.rows() - 15, P_corrected.cols() - 15);
        ...
        imuState <_S> propogateImuStateRK(const imuState <_S> &imu_state_k, const imuReading <_S> &measurement_k) {
            ...
            k5 = omega_psi * (y0 + (-k0 * 3. / 7. + k1 * 2. / 7. + k2 * 12. / 7. - k3 * 12. / 7. + k4 * 8. / 7.) * dT);
        ...
        void removeTrackedFeature(
            ...
            for (size_t c_i = 0; c_i < cam_states_.size(); c_i++) {
                auto feature_iter =
                        std::find(cam_states_[c_i].tracked_feature_ids.begin(), cam_states_[c_i].tracked_feature_ids.end(), featureID);
