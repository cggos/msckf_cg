...
namespace penncosy_dataset {
    ...
    Sensor::Sensor(std::string folder, std::string sensor_name, std::string seq, std::string csv_name,
                   std::string sub_sensor_name) :
            sensor_name_(sensor_name), sequence_(seq), folder_(folder), sub_sensor_name_(sub_sensor_name),
            csv_name_(csv_name), cur_time_(0) {
    }
    ...
    cv::Mat VICamera::get_data() {
        ...
        stream << folder_ << "/data/" << sensor_name_ <<
               ...
               "/frame_" << std::setfill('0') << std::setw(4) << (size_t) (list_iter_ - reading_list_.begin()) + 1
               << ".png";
