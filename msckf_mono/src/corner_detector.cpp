...
namespace corner_detector {
    ...
    float CornerDetector::shiTomasiScore(const cv::Mat &img, int u, int v) {
    ...
    CornerTracker::CornerTracker(int window_size,
            ...
            : window_size_(window_size, window_size),
              ...
              termination_criteria_(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, termcrit_max_iters,
                                    termcirt_epsilon) {
    }
    ...
    void CornerTracker::track_features(
            cv::Mat img_1, cv::Mat img_2,
            Point2fVector &points1, Point2fVector &points2,
            IdVector &id1, IdVector &id2) {
    ...
    TrackHandler::TrackHandler(const cv::Mat K, const cv::Mat distortion_coeffs, const std::string dist_model)
    ...
    void TrackHandler::tracked_features(OutFeatureVector &features, IdVector &feature_ids) {
        ...
        if (cur_features_.size() > 0) {
            ...
            features.clear();
            std::copy(cur_features.begin(), cur_features.end(), std::back_inserter(features));

            feature_ids.clear();
            std::copy(cur_feature_ids_.begin(), cur_feature_ids_.end(), std::back_inserter(feature_ids));
        }
    }
    ...
    void TrackHandler::new_features(OutFeatureVector &features, IdVector &feature_ids) {
        ...
        if (new_features_.size()) {
            ...
            std::copy(new_feature_ids_.begin(), new_feature_ids_.end(), std::back_inserter(feature_ids));
        }
    }
