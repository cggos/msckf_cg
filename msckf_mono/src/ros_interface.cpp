...
namespace msckf_mono {
    RosInterface::RosInterface(ros::NodeHandle nh) :
        ...
        image_sub_ = it_.subscribe("image_mono", 20, &RosInterface::imageCallback, this);
    }
    ...
    void RosInterface::imageCallback(const sensor_msgs::ImageConstPtr &msg) {
        ...
        std::vector<Vector2<float>, Eigen::aligned_allocator<Vector2<float>>> cur_features;
        corner_detector::IdVector cur_ids;
        track_handler_->tracked_features(cur_features, cur_ids);

        std::vector<Vector2<float>, Eigen::aligned_allocator<Vector2<float>>> new_features;
